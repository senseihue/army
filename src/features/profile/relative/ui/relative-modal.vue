...
import { Relative } from "~/entities/profile/relative"
import { useRelativeService } from "~/features/profile/relative"
import { KinshipSelect } from "~/widgets/references/kinship"
import UiDatePicker from "@vuepic/vue-datepicker"

const { saveRelative } = useRelativeService()
...
const form = ref<Relative>(new Relative())
const rules = ref({
  kinship_id: { required },
  birth_date: { required },
  last_name: { required },
...
const onShown = (data: IRelative) => {
  if (data?.id) {
    editing.value = true
    form.value = data
  } else {
    editing.value = false
    form.value = new Relative()
    vuelidate.value.$reset()
  }
}
const submit = async () => {
  const valid = await vuelidate.value.$validate()
  if (valid) await saveRelative(form, loading)
}
...
<template>
  <ui-modal id="relative" :loading :label @shown="onShown">
    <form class="grid grid-cols-1 md:grid-cols-2 gap-4 px-4 py-[15px]" @submit.prevent>
      <ui-form-group v-bind="hasError('kinship_id')" v-slot="{ id }" :label="t('labels.kinship')">
        <kinship-select v-model="form.kinship_id" :id></kinship-select>
      </ui-form-group>
