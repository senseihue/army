<template>
  <div class="mx-auto w-full max-w-[1088px] px-4 p-section">
    ...
    <div v-if="loading" role="status">
      <svg
        class="mx-auto h-12 w-12 animate-spin fill-blue-command text-gray-200 dark:text-gray-600"
        aria-hidden="true"
        viewBox="0 0 100 101"
        fill="none"
...
import { ProjectCategoryCard, useProjectCategoryService } from "~/features/project/category"

const { t } = useI18n({ useScope: "local" })
const projectCategoryService = useProjectCategoryService()
const loading = ref<boolean>(false)
const list = ref<IProjectCategory[]>([])

onMounted(() => projectCategoryService.getProjectCategoryList({}, list, loading))
</script>
