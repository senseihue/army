...
const map = (value: IProjectCategorySector[]): ISelect[] =>
  value?.map(({ id, content }) => ({ value: id, label: content }))

const { loading, onOpen, onClose, onSearch, options } = useSelect<IProjectCategorySector>({
  el,
  map,
  model,
  params,
  ...
  api: projectCategoryApi.getServiceSector
})
</script>
